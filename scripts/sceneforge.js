...
async function createSceneFromGenerationData(generationData, seedWasAutoGenerated = false, options = {}) {
  ...
  if (isAiPlannerMode) {
    const provider = imageGenerationMetadata?.provider ?? getAiImageProvider();
    ...
    const hasValidAiImage = (
      (provider === "openai" && imageStatus === "complete")
      || (provider === "mock" && imageStatus === "mock-generated")
    ) && Boolean(imagePath);
...
function buildScenePresetPayload(scene, generationData) {
  return {
    ...
    version: generationData.moduleVersion ?? "0.14.4",
